...
contract Sushiswap_YZapOut is ReentrancyGuard, Ownable {
    using SafeMath for uint256;
    ...
    IUniswapV2Router02 private uniswapRouter;//0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D 
    IUniswapV2Factory private UniSwapV2Factory;//0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f

    // sushiSwap
    IUniswapV2Router02 private sushiSwapRouter;//0xd9e1cE17f2641f24aE83637ab66a2cca9C378B9F
    IUniswapV2Factory private sushiSwapFactory;//0xC0AEe478e3658e2610c5F7A4A2E1777cE9e4f2Ac
    ...
    /**
    @notice This function is used to yzapout of given SushiSwap pair in the bounded tokens
    @param _FromSushiPoolAddress The sushiSwap pair address to yzapout
    @param _IncomingLP The amount of LP
    @return the amount of pair tokens received after yzapout
     */
    function YZapOut2PairToken(
    ...
    /**
    @notice This function is used to yzapout of given Sushiswap pair in ETH/ERC20 Tokens
    @param _ToTokenContractAddress The ERC20 token to yzapout in (address(0x00) if ether)
    @param _FromSushiPoolAddress The sushiswap pair address to yzapout from
    @param _IncomingLP The amount of LP
    ...
    function YZapOut(
        ...
        if (_ToTokenContractAddress == address(0)) {
            //toWhomToIssue.transfer(tokenBought);
            Address.sendValue(toWhomToIssue, tokenBought);
        } else {
    ...
    function getTokenAddressFromSwapAddress(address pairAddress) external pure returns (address) {
        return pairAddress;
    }
    /** End Interface functions **/

    function YZapOut2PairTokenWithPermit(
        address _FromSushiPoolAddress,
        uint256 _IncomingLP,
        ...
        (amountA, amountB) = YZapOut2PairToken(
            _FromSushiPoolAddress,
            _IncomingLP
        );
    }

    function YZapOutWithPermit(
        address _ToTokenContractAddress,
        address _FromSushiPoolAddress,
        uint256 _IncomingLP,
        uint256 _minTokensRec,
    ...
    function swapFromV2(
        ...
        if (_fromToken == address(0)) {
            ...
            path[1] = _toToken;
            uint256 minTokens = uniswapRouter.getAmountsOut(amount, path)[1];
            ...
            address[] memory path = new address[](2);
            IERC20(_fromToken).safeApprove(address(uniswapRouter), amount);
            path[0] = _fromToken;
    ...
    function _swapTokenToTokenV2(
        ...
        IUniswapV2Pair pair1 = IUniswapV2Pair(
            UniSwapV2Factory.getPair(_fromToken, wethTokenAddress)
        );
        IUniswapV2Pair pair2 = IUniswapV2Pair(
            UniSwapV2Factory.getPair(_toToken, wethTokenAddress)
        );
        IUniswapV2Pair pair3 = IUniswapV2Pair(
            UniSwapV2Factory.getPair(_fromToken, _toToken)
        );
        ...
        if (_haveReserve(pair3)) {
            ...
            path[1] = wethTokenAddress;
            path[2] = _toToken;
            uint256 minTokens = uniswapRouter.getAmountsOut(amount, path)[2];
            minTokens = SafeMath.div(
                SafeMath.mul(minTokens, SafeMath.sub(10000, 200)),
                10000
            );
