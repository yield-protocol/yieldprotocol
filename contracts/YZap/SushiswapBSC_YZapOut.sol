...
contract SushiswapBSC_YZapOut is ReentrancyGuard, Ownable {
    using SafeMath for uint256;
    ...
    IUniswapV2Router02 private constant pancakeswapV2Router = IUniswapV2Router02(
        0x10ED43C718714eb63d5aA57B78B54704E256024E
    );//PancakeSwap: Router 
    IUniswapV2Factory
        private constant PancakeSwapV2FactoryAddress = IUniswapV2Factory(
    ...
    function swapFromV2(
        ...
        if (_fromToken == address(0)) {
            ...
            path[1] = _toToken;
            uint256 minTokens = pancakeswapV2Router.getAmountsOut(amount, path)[1];
            minTokens = SafeMath.div(
                SafeMath.mul(minTokens, SafeMath.sub(10000, 200)),
                10000
            );
            uint256[] memory amounts = pancakeswapV2Router
            ...
            address[] memory path = new address[](2);
            IERC20(_fromToken).safeApprove(address(pancakeswapV2Router), amount);
            path[0] = _fromToken;
            ...
            uint256[] memory amounts = pancakeswapV2Router.swapExactTokensForETH(
                amount,
                minTokens,
                path,
                address(this),
    ...
    function _swapTokenToTokenV2(
        ...
        IUniswapV2Pair pair1 = IUniswapV2Pair(
            PancakeSwapV2FactoryAddress.getPair(_fromToken, wethTokenAddress)
        );
        IUniswapV2Pair pair2 = IUniswapV2Pair(
            PancakeSwapV2FactoryAddress.getPair(_toToken, wethTokenAddress)
        );
        IUniswapV2Pair pair3 = IUniswapV2Pair(
            PancakeSwapV2FactoryAddress.getPair(_fromToken, _toToken)
        );
        ...
        if (_haveReserve(pair3)) {
            ...
            path[1] = wethTokenAddress;
            path[2] = _toToken;
            uint256 minTokens = pancakeswapV2Router.getAmountsOut(amount, path)[2];
            minTokens = SafeMath.div(
                SafeMath.mul(minTokens, SafeMath.sub(10000, 200)),
                10000
            );
