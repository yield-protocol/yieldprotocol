...
contract UniswapV2_YZapOut is ReentrancyGuard, Ownable {
    using SafeMath for uint256;
    ...
    /**
    @notice This function is used to yzapout of given Uniswap pair in the bounded tokens
    @param _FromUniPoolAddress The uniswap pair address to yzapout
    @param _IncomingLP The amount of LP
    @return the amount of pair tokens received after yzapout
     */
    function YZapOut2PairToken(address _FromUniPoolAddress, uint256 _IncomingLP)
        public
        nonReentrant
        stopInEmergency
    ...
    @param _FromUniPoolAddress The uniswap pair address to yzapout from
    @param _IncomingLP The amount of LP
    ...
    /*function unstakeAndZapOut(address toTokenAddress, address stakeContractAddress) external returns (uint256){
        INoMintRewardPool stakeContract = INoMintRewardPool(stakeContractAddress);
        address pairAddress = stakeContract.lpToken();
        //stakeContract.exit(); 
        bool status;
        bytes memory result;
        (status, result) = stakeContractAddress.delegatecall(abi.encodeWithSignature("exit()"));
        return YZapOut(toTokenAddress, pairAddress, IERC20(pairAddress).balanceOf(msg.sender), 1);
    }*/
    ...
    function YZapOut2PairTokenWithPermit(
        ...
        (amountA, amountB) = YZapOut2PairToken(_FromUniPoolAddress, _IncomingLP);
    }
