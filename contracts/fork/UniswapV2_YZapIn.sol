...
contract UniswapV2_YZapIn is ReentrancyGuard, Ownable {
    using SafeMath for uint256;
    using Address for address;
    ...
    IUniswapV2Router02 private uniswapRouter = IUniswapV2Router02(
        routerAddress
    );

    IUniswapV2Factory
        private constant UniSwapV2Factory = IUniswapV2Factory(
    ...
    constructor(address _weth, address _uniswapFactory, address _uniswapRouter) public {
        wethTokenAddress = _weth;
        UniSwapV2Factory = IUniswapV2Factory(_uniswapFactory);
    ...
    function YZapInTokens(
        ...
        uint256 LPBought = _performYZapIn(
            _toWhomToIssue,
        ...
        //get pair address
        address _ToUniPoolAddress = UniSwapV2Factory.getPair(
            _ToUnipoolToken0,
            _ToUnipoolToken1
        );
    ...
    /*function convertTokenToYIELD(address _toWhomToIssue, address _tokenToExchange, address _yieldToken, uint256 _amount) external returns (uint256) {

        IERC20(_tokenToExchange).approve(
            routerAddress,
            _amount
        );
    ...
    function convertTokenToETH(address _toWhomToIssue, address _tokenToExchange, uint256 _amount) external returns (uint256) {

        IERC20(_tokenToExchange).approve(routerAddress, _amount);
    ...
    /*function YZapInAndStake(
        ...
        INoMintRewardPool stakeContract = INoMintRewardPool(stakeContractAddress);
        address pairAddress = stakeContract.lpToken();
        IUniswapV2Pair uniPair = IUniswapV2Pair(pairAddress);

        //note: update YZapInTokens functin to also accept pair
        uint256 lpAmount = YZapInTokens(msg.sender, fromTokenAddress, uniPair.token0(), uniPair.token1(), amount, minPoolTokens);
        ...
        bool status;
        bytes memory result;
        (status, result) = stakeContractAddress.delegatecall(abi.encodeWithSignature("stake(uint256)", lpAmount));
        //require(status == true, "status is not true");
        //stakeContract.stake(lpAmount);
        //stakeContractAddress.delegatecall(bytes4(keccak256("stake(uint256)")), lpAmount);
        return lpAmount;
    }*/
    ...
    /*function YStake(
        ...
        IERC20(lpTokenAddress).approve(
            stakeContractAddress,
            lpTokenAmount
        );

        INoMintRewardPool stakeContract = INoMintRewardPool(stakeContractAddress);
        stakeContract.stake(lpTokenAmount);

        //stakeContractAddress.delegatecall(abi.encodeWithSignature("stake(uint256)", _to[i], _value[i]));

    }*/

    /** End Interface functions **/

    function _performYZapIn(
        address _toWhomToIssue,
        address _FromTokenContractAddress,
        address _ToUnipoolToken0,
        address _ToUnipoolToken1,
        ...
        IUniswapV2Pair pair = IUniswapV2Pair(
            UniSwapV2Factory.getPair(_ToUnipoolToken0, _ToUnipoolToken1)
        );
    ...
    function _calculateSwapOutput(
        ...
        address pairA = UniSwapV2Factory.getPair(_from, _to);
    ...
    function _token2Token(
        ...
        if (_FromTokenContractAddress == address(0)) {
            ...
            if (_FromTokenContractAddress != wethTokenAddress) {
                if (_ToTokenContractAddress != wethTokenAddress) {
                    // check output via tokenA -> tokenB
                    address pairA = UniSwapV2Factory.getPair(
                        _FromTokenContractAddress,
